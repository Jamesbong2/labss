<template lang="">
  <div class="p-10 w-[500px] space-y-10 m-auto">
    ...
    <div class="">
      ...
      <input class="input" v-model="form.materials" id="materials" placeholder="Имя" type="text" />
    </div>
    <Btn @click="onClick" :disabled="!isValidForm" :isInfo="true">Сохранить</Btn>
  </div>
</template>
<script>
import { fetchItems, selectItemById, selectItems } from '@/store/materials/service';
...
export default {
  ...
  setup(props, context) {
    ...
    return {
      groupList,
      form,
      isValidForm: computed(() => !!form.material),
      onClick: () => {
